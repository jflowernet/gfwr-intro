...
What if we want to find info for a particular vessel? We can do that using 


```{r}
#| eval: false

#which vessel did most fishing in 2022
...
fishing_events_sf <- st_as_sf(fishing_events, 
         coords = c("lon", "lat"),
         crs = 4326) 
  
plot(st_geometry(fishing_events_sf))

vessel_tracks <- fishing_events_sf %>%
  arrange(end) %>%
  mutate(month = months(end)) %>%
  group_by(month) %>% 
  summarise() %>%
...
ggplot() +
  geom_sf(data = vanuatu_eez) +
  geom_sf(data = vessel_tracks,aes(col = month))
```
